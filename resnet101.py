...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.conv2 = nn.Conv2d(self.inplanes, 128, kernel_size=1, stride=1, padding=0, bias=False)
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2])  # basic, 512, 2, 2, false    [-1, 2048, 11, 15]

        # self.conv4 = nn.Conv2d(2048, 256, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def _forward_impl(self, x):
        ...
        # x = torch.flatten(x, 1)
        # x = self.fc(x)
        x = F.interpolate(x, size=(22,30), mode='bilinear', align_corners=True)
...
if __name__ == "__main__":
    resnet_101_model = resnet101()

    print(summary(resnet_101_model, input_size=(3, 352, 480)))
    #model = resnet_101_model(nInputChannels=3, n_classes=12, os=16, _print=True)
    resnet_101_model.eval()
    image = torch.randn(1, 3, 352, 480)
    x,low_level_feat = resnet_101_model(image)
    print(low_level_feat.size())

    print(x.shape)  #torch.Size([1, 2048, 22, 30])
